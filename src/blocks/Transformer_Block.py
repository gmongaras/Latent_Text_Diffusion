...
class Transformer_Block(nn.Module):
    def __init__(self, dim, c_dim, hidden_scale=4.0, num_heads = 8, attn_type = "softmax", causal=False, positional_encoding="absolute", layer_idx=None):
        super().__init__()
    ...
    def forward(self, X, y=None, mask=None):
        # Attn layer
        X = (self.attn(self.norm1(X, y), mask=mask) * self.scale1(y)[:, None, :]) + X
